...
function prepareRow(item) {
  var row = '<tr>';
  if(item.Type === 'directory'){
    row += `<td><img src='./foldericon.png'></td>`
  }else{
    row += `<td><img src='./file.png'></td>`
  }
  
  row += `<td style="padding-left: 20px;"><a href=${item.href}>${item.keyText}</a></td>`;
  row += `<td style="padding-left: 20px;">${item.LastModified}</td>`;
  row +=`<td style="padding-left: 20px;">${item.Size}</td>`;
  row += '</tr>'
  return row;
}
